...
export async function searchGames(name: string) {
  ...
  const query = `search "${safe}"; fields id, name, first_release_date; limit 20; `;
...
export async function searchGameInfo(igdbId: number) {
  const query = `fields id, name, artworks, first_release_date, cover.image_id, summary, involved_companies; where id = ${igdbId}; limit 1;`;
  // const query = `search "${safe}"; fields id, name, first_release_date; limit 20; `;

  const result = await igdbGames(BASE_URL, query);
  const game = result?.[0];
  if (!game) return null;

  const artworkIds: number[] = Array.isArray(game.artworks)
    ? game.artworks
    : [];
  const involvedIds: number[] = Array.isArray(game.involved_companies)
    ? game.involved_companies
  ...
  const [artworks, involvedCompanies] = await Promise.all([
    fetchArtworks(artworkIds),
    fetchDevelopers(involvedIds),
  ]);
  ...
  const artworksUrls: string[] = artworkIds
    .map((id) => artworkById.get(id))
    .filter(Boolean)
    .map((a) => (a.image_id ? igdbImageUrl(a.image_id, "t_1080p") : null))
    .filter((u): u is string => Boolean(u));
  ...
  const developers = Array.from(
    new Set(
      involvedIds
        ...
        .map((row) => row.company?.name ?? "Unknown"),
    ),
  );
